...
export default class FormValidator {
    ...
    switchButton(allInputs, buttonElement) {
         if (this._checkInvalidInput(allInputs) || this._allInputsEmpty(allInputs)) {
            buttonElement.classList.add(this._validationConfig.disableSubmitButton);
            buttonElement.setAttribute('disabled', true);
         } else {
            buttonElement.classList.remove(this._validationConfig.disableSubmitButton);
    ...
    _inputsListeners(){
         // получаем все импуты в переменной allInputs
           const allInputs = Array.from(this._formValidate.querySelectorAll(this._validationConfig.inputSelector));
         // кнопка сохранить, добавить
           const buttonElement = this._formValidate.querySelector(this._validationConfig.submitButton);
         // проходим по всем импутам методом forEach и навешиваем обработчи событий
